...
本节给出本文研究对象的形式化定义。本文把测试时计算视为可记账的资源分配问题：在同一代码生成任务集合上，给定一个预算档位，系统决定是否继续调用模型、如何使用执行反馈、是否扩展候选集，并在统一执行器下记录收益与成本。这一视角与预算预测和自适应推理中的成本约束思想一致，即不同样本可以消耗不同数量的专家查询或网络计算\cite{amin2015budgeted,bolukbasi2017adaptive}。
...
\begin{tabularx}{\linewidth}{@{}p{1.15cm} p{2.1cm} X@{}}
\toprule
语义档位 & 实现模式 & 允许的主要计算投向 \\
\midrule
单步 & \texttt{single\_step} & 单次生成与执行判定；作为最低成本基线 \\
单步+修补 & \makecell[l]{\texttt{single\_step}\\\texttt{\_repair}} & 在单次失败后使用执行反馈做有限修补 \\
\makecell[l]{分流+重链\\与救援} & \makecell[l]{\texttt{difficulty}\\\texttt{\_aware\_plus}} & 依据结构难度进入轻/重路径，重路径启用结构化主链与失败后救援 \\
级联 & \makecell[l]{\texttt{workflow}\\\texttt{\_cascade}} & 多候选扩展、执行筛选以及有限精修/融合 \\
\bottomrule
\end{tabularx}
\end{table}
